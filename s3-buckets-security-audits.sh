...
# Attribution, from: https://github.com/swoodford/aws/blob/master/s3-buckets-security-audit.sh

# Script to export S3 bucket ACL, CORS, Policy, Website, and Public Block Access JSON for auditing security of all buckets
...
# Lookup the get-public-access-block policy
  PUBLICBLOCK=$(aws s3api get-public-access-block --bucket $CURRENTBUCKET --region $REGION --profile $profile 2>&1)

  if echo "$PUBLICBLOCK" | grep -q "error"; then
    PUBLICBLOCK='{
    "Public Access Block": [
    ]
}'
  ...
  OUTPUT=$(echo $ACL $CORS $POLICY $WEBSITE $PUBLICBLOCK | jq -s add)
